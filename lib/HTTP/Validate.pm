...
This key specifies the domain of acceptable values for the parameter.  The
...
If this directive is used, then the cleaned value of the parameter will be an
array ref if at least one valid value was found and I<undef> otherwise.  If
...
my ($DEFAULT_INSTANCE) = bless { RULESETS => {}, SETTINGS => {} };
...
HTTP::Validate and use any of the routines listed below as methods:
    ...
    my $vv = HTTP::Validate->new();
    
    $vv->define_ruleset('my_params' =>
        { param => 'foo', valid => INT_VALUE, default => '0' });
    
    my $validated = $vv->check_params('my_params', \%ARGS);
...
Any parameter rules that you wish to leave undocumented should be immediately
followed by a string starting with "!".  All others will automatically
generate list items in the resulting documentation, even if no documentation
string is provided (in this case, the item body will be empty).
...
sub add_rules {
  ...
  RULE:
	...
	unless ( ref $rule )
	{
		...
		if ( $1 eq '>>' )
		{
		    $self->add_doc($rs, $doc_rule, @doc_lines) if $doc_rule;
		    @doc_lines = $2;
		    $doc_rule = undef;
		}
	...
	{
	    croak "unknown key '$key' found in rule" unless $DIRECTIVE{$key};
	    
	    if ( $DIRECTIVE{$key} == 2 )
	    {
		croak "a rule definition cannot contain the keys '$key' and '$type' together, because they indicate different rule types"
		    if $type;
		$type = $key;
		$rr->{$type} = $rule->{$type};
		next KEY;
	...
	{	
	    ...
	    if ( $key eq 'valid' )
	    {
		croak "the key 'valid' is only allowed with parameter rules"
		    unless $CATEGORY{$type} eq 'param' || $type eq 'content_type';
	    }
	    
	    elsif ( $key eq 'alias' )
	    {
		croak "the key 'alias' is only allowed with parameter rules"
	    ...
	    {
		croak "the key '$key' is only allowed with parameter rules"
		    unless $CATEGORY{$type} eq 'param';
	...
	# If we have any documentation strings collected up, then they belong to
	# the previous rule.  If the current rule is not a modifier-type rule,
	# then add the collected documentation to the previous rule and set
	# this new rule as the target for subsequent documentation.
	...
	elsif ( $CATEGORY{$type} eq 'modifier' )
	{
	    ...
	    {
		croak "parameter '$arg' was not defined" unless defined $rs->{params}{$arg} ||
		    $type eq 'ignore';
		push @{$rr->{param}}, $arg;
	    }
...
sub add_doc {
	...
	push @{$rs->{doc_items}}, process_doc($body);
    }
...
sub new_execution {
    ...
    my $vr = { raw => $unpacked_params,	# the raw parameters and values
	       clean => {},		# the cleaned parameters and values
...
sub execute_validation {
	...
	my $message = @names == 1 ? "you must include the parameter {param}" :
	    "you must include at least one of the parameters {param}";
	add_error($vr, { key => $ruleset_name }, $message, { param => \@names });
    }
...
sub validate_ruleset {
 ...
 RULE:
	...
	if ( $type eq 'param' )
	{
	    ...
	    if ( @names_found > 1 && ! $rr->{multiple} )
	    {
		add_error($vr, $rr, "you may only include one of {param}",
			  { param => \@names_found });
		next RULE;
	    }
	    ...
	    {
		add_error($vr, $rr, "you may only specify one value for {param}: found {value}",
			  { param => @names_found, value => \@raw_values });
		next RULE;
	    }
	...
	VALUE:
		...
		if ( ref $result and $result->{error} )
		{
		    ...
		    if ( $rr->{warn} )
		    {
			add_warning($vr, $rr, $result->{error}, { param => @names_found, value => $raw_val });
		    }
		    ...
		    {
			add_error($vr, $rr, $result->{error}, { param => @names_found, value => $raw_val });
		    }
		...
		{
		    add_warning($vr, $rr, $result->{warn}, { param => @names_found, value => $raw_val });
		}
	    ...
	    # good values were found.  Otherwise, store nothing.
	    
	    if ( $rr->{multiple} )
	    {
		$vr->{clean}{$key} = \@clean_values if @clean_values;
	    }
	    
	    # Otherwise, if the 'allow_multiple' setting is active, then store
	    # a list if more than one good value was found and a single
	    # value otherwise.
	    
	    elsif ( $self->{SETTINGS}{allow_multiple} )
	    {
		$vr->{clean}{$key} = @clean_values > 1 ? \@clean_values : $clean_values[0];
	    }
	    
	    # Otherwise, if a good value was found then store it.
	...
	elsif ( $rr->{type} eq 'together' or $rr->{type} eq 'at_most_one' )
	{
	    ...
	    {
		add_error($vr, $rr, "you must include {param} together or not at all",
			  { param => $rr->{param} });
	    }
	    ...
	    {
		add_error($vr, $rr, "you may not include more than one of {param}",
			  { param => \@present });
	    }
	}
	...
	elsif ( $rr->{type} eq 'include' )
	{
		...
		my $message = @missing == 1 ? 
		    "you must include the parameter {param}" :
			"you must include one of the parameters {param}";
		add_error($vr, $rr, $message, { param => \@missing });
	    }
	}
	...
	elsif ( $rr->{type} eq 'constraint' )
	{
	    ...
	    {
		my (@params) = map { $self->{RULESETS}{$_}{fulfill_order} } @fulfilled;
		my ($label) = "A";
		
		my (@groups) = map { '(' . $label++ . ') ' . name_list(@$_) } @params;
		my $message = @groups == 2 ?
		    "you may not include parameters from both of these groups: {param}" :
			"you may not include parameters from more than one of these groups: {param}";
		
		add_error($vr, $rr, $message, { param => join('; ', @groups) });
	    }
	}
	...
	elsif ( $type eq 'content_type' )
	{
		...
		add_error($vr, $rr,
			  "your request must specify a media type from the following list: {param}",
			  { param => $rr->{type_map} });
	    }
	}
    }
...
# Add an error message to the current validation.  The @subst arguments if
# given specify placeholder substitutions.

sub add_error {

    my ($vr, $rr, $msg, $subst) = @_;
    
    # If the rule has a 'warn' directive, add a warning instead.
    
    goto &add_warning if $rr->{warn};
    
    # If the rule has an 'errmsg' directive, use that message instead of the
    # one given.
    
    $msg = $rr->{errmsg} if $rr->{errmsg};
    
    # If no message was given, use a default one.
    
    $msg ||= "parameter value error";
...
# Add a warning message to the current validation.  The @subst arguments if
# given specify placeholder substitutions.
...
sub add_warning {
    ...
    $msg ||= "parameter value warning";
    
    # If the rule has an 'warn' directive, use that message instead of the one
    # given.
    
    if ( $rr->{warn} )
    {
	$msg = $rr->{warn} ne '1' ? $rr->{warn} : $msg;
    }
...
In a scalar context, this returns the number of parameters that had valid
values.  In a list context, it returns a list of parameter names.  Individual
...
sub keys {
    ...
    # All of the keys of the 'values' subhash represent clean parameter values.
...
rejects any that does not.  I<The pattern is automatically modified by
prepending ^ and appending $, and by adding the modifier "i">.  Any rule that
uses this validator should be provided with an error directive, since the
...
sub MATCH_VALUE {
    ...
    return sub { return match_value(shift, shift, qr{^$pattern$}oi) };
};
...
sub ENUM_VALUE {
    ...
    croak "ENUM_VALUE requires at least one value" unless @non_empty;
    
    my %accepted = map { $case_fold->($_) => $_ } @non_empty;
    my $good_list = "'" . join("', '", @non_empty) . "'";
